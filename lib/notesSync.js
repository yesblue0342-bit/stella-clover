...
// notes_meta.body 도 여기서 함께 채운다(readJsonById 로 이미 전체 JSON을 받아오므로 추가 Drive
// 호출 없음) — Stella GPT 등 외부에서 고친 본문이 5분 내 Clover 상세 열람 캐시에도 반영된다.
import { listJsonInFolder, listJsonInFolderSince, readJsonById } from "../api/_drive.js";
import { getSyncPool } from "../api/_db.js";
...
async function upsertRow(pool, row) {
  await pool.request()
    ...
    .query(`
      ...
      VALUES (@id, @driveFileId, @title, @preview, @body, @source, @updatedAt, @deletedAt)
      ON CONFLICT (id) DO UPDATE SET
        drive_file_id = @driveFileId, title = @title, preview = @preview, body = @body,
        source = @source, updated_at = @updatedAt, deleted_at = @deletedAt
    `);
}

// note.body 는 이미 readJsonById 로 받아온 전체 JSON에서 나온다 — preview 만 쓰고 버리던 것을
// notes_meta.body 에도 함께 저장(추가 Drive 호출 없음). 상세 열람(action=get)의 Postgres 캐시 히트 소스.
