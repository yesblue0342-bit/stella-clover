...
// 동시성 제한 병렬 map — worker(item, index)를 최대 limit개까지 동시에 실행하고 결과를 입력 순서대로
// 반환한다. 코드 리뷰의 (파일×청크) LLM 호출을 병렬화해 순차 대비 크게 단축하는 데 쓴다.
// worker가 throw하면 그 rejection이 전파된다(호출부에서 try/catch로 부분 실패를 흡수).
export async function mapWithConcurrency(items, limit, worker) {
  const list = Array.from(items || []);
  const results = new Array(list.length);
  if (!list.length) return results;
  const n = Math.max(1, Math.min(list.length, Number(limit) || 1));
  let next = 0;
  async function run() {
    while (true) {
      const i = next;
