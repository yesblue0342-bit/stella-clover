// lib/ai-connection/providers.js — AI 연결(API 키 / ChatGPT·Claude 구독 CLI) 공용 모듈.
// 원래 lib/cbo-review/providers.js였던 것을 2026-07-14 통합 세션에서 이 위치로 이동했다 — CBO Review와
// CBO Pre-Check가 동일 파일을 참조해 "완전히 동일한 연결 상태"를 공유한다(WORK_REPORT.md Phase 1 참고).
...
// DATA_DIR 기본값은 이동 전과 동일하게 유지한다(운영 서버에 이미 저장된 provider 설정을 잃지 않기 위해 —
// 파일 위치는 저장소 구현 세부사항일 뿐이다).
...
// CLI 서브프로세스 하드킬 타임아웃. xlsx 첨부 2개+SAP QM CBO FS 작성 같은 실사용 요청은 5~10분이
// 정상 소요라, 과거 180초/240초 상한은 정상 요청도 타임아웃으로 실패시켰다(REVIEW_LOG.md 참조).
// 이제 CBO 요청 자체가 비동기 잡(lib/cbo-review/jobRuntime.js)으로 처리되어 HTTP 응답 시간과
// 무관하므로, 실사용 소요시간에 여유를 둔 15분으로 상향한다.
...
function defaultModelFor(provider, status) {
  return status?.models?.[0] || PROVIDER_MODELS[provider]?.[0] || "";
}

function fallbackMessage(error) {
  return String((error && error.message) || error || "").replace(/\s+/g, " ").slice(0, 220);
}

// 선택한 provider가 CLI 구독 정책/만료/키 문제로 실패해도, 사용자가 다른 provider를 이미 연결해 둔 경우
// 같은 요청을 자동으로 이어서 시도한다. CBO Review/Pre-Check의 "둘 다 연결했는데 한쪽 실패가 전체 실패" 회귀 방지.
export async function callModelWithFallback({ provider, model, system, user, json = false }) {
  validateProviderModel(provider, model);
  ...
  for (const p of ["openai", "anthropic", "gemini"]) {
    if (p === provider || !byProvider[p]?.connected) continue;
    const fallbackModel = defaultModelFor(p, byProvider[p]);
  ...
  for (const candidate of candidates) {
    try {
      const text = await callModel({ ...candidate, system, user, json });
      if (candidate.provider !== provider) {
        console.warn(`[ai-connection] ${provider}/${model} 실패 후 ${candidate.provider}/${candidate.model}로 폴백 성공`);
      }
      return text;
    } catch (error) {
      errors.push(`${candidate.provider}/${candidate.model}: ${fallbackMessage(error)}`);
    }
  }
