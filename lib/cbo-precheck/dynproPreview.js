...
// FORM <name> USING <params>.<body>ENDFORM. 공통 스캔 — findFieldcatForms와 (구)findFormsWithUsingParams가
// 각자 같은 정규식/파라미터 추출 로직을 들고 있다가 한쪽만 확장되고 다른 쪽이 누락되는 사고(예:
// extractSetDirective의 WITH-그룹 결함과 같은 유형)를 막기 위해 여기서 한 번만 정의한다. 향후 CHANGING/
// TABLES 파라미터나 멀티라인 시그니처를 지원하게 되면 이 함수 하나만 고치면 두 호출부 모두에 반영된다.
function scanFormsWithUsingParams(source) {
  const forms = [];
...
function findFieldcatForms(source) {
  const forms = [];
  for (const { name, paramNames, body } of scanFormsWithUsingParams(source)) {
...
export function extractSalvColumnTexts(source) {
  ...
  for (const form of scanFormsWithUsingParams(source)) {
