// api/meetings.js - 회의록 목록 + 키워드 검색 + 상세 (PostgreSQL)
import { getPool, sql, hasDbConfig } from "./_db.js";

// 연결 타임아웃 30s + 재시도를 함수 한도 안에 수용하려면 여유 필요.
...
export default async function handler(req, res) {
  ...
  if (!hasDbConfig()) {
    return res.status(200).json({ ok: false, items: [], message: "DB 환경변수 미설정 (DATABASE_URL 또는 PGHOST 확인)" });
  }
  ...
  try {
    ...
    if (action === "search") {
      ...
      // ILIKE 와일드카드(%, _)와 이스케이프 문자(\) 를 리터럴로 이스케이프(기본 ESCAPE '\\').
