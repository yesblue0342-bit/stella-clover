...
import { isDictionaryDoc } from "./dictParser.js";
import { isTextsDoc } from "./textSymbols.js";
import { isRfcDdicDoc } from "./abapObject.js";
...
function isTargetFile(name) {
  ...
  const isDict = isDictionaryDoc(name);
  const isTexts = isTextsDoc(name);
  const isRfcDdic = isRfcDdicDoc(name);
  return { isAbap, isDdic, isDict, isTexts, isRfcDdic, isTarget: isAbap || isDdic || isDict || isTexts || isRfcDdic };
}

// ABAP 소스(.abap) + DDIC XML(abapGit TABL/DTEL/DOMA/...) + dictionary 문서(dictionary/*.md|*.html,
// 합성 DDIC 생성용 — lib/cbo-precheck/dictParser.js)만 수집. 그 외 파일은 스캔 대상이 아니다.
...
export async function collectAbapFiles(root) {
  ...
  if (rootStat && rootStat.isFile()) {
    const { isTarget, isDdic, isDict, isTexts, isRfcDdic } = isTargetFile(path.basename(root));
    if (isTarget && rootStat.size <= MAX_FILE_BYTES) {
      const content = await fs.readFile(root, "utf8");
      files.push({ name: path.basename(root), content, isDdic, isDict, isTexts, isRfcDdic });
    }
    return files;
  }
  ...
  async function walk(dir) {
    ...
    for (const entry of entries) {
      ...
      const relName = path.relative(root, full).replaceAll("\\", "/");
      // isDict(dictionary/*.md|*.html) 판별은 디렉토리 경로가 필요하므로 basename이 아니라 relName으로
      // 검사한다(entry.name만 쓰면 "dictionary/" 세그먼트가 없어 항상 false — 실측으로 발견한 버그).
      const { isTarget, isDdic, isDict, isTexts, isRfcDdic } = isTargetFile(relName);
      if (!isTarget) continue;
      const stat = await fs.stat(full);
      if (stat.size > MAX_FILE_BYTES) continue;
      const content = await fs.readFile(full, "utf8");
      files.push({ name: relName, content, isDdic, isDict, isTexts, isRfcDdic });
    }
  }
  await walk(root);
  return files;
