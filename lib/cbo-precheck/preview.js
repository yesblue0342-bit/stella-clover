...
import { isAbapGitNamed, detectAbapGitType } from "./scan.js";
...
// plain 네이밍(`ZAQMR0130_S01.abap`)이면 abaplint가 타입을 인식 못 해 UnknownObject로 떨어져
// getABAPFiles()가 없다(elements:[] 로 조용히 비게 됨 — lib/cbo-precheck/scan.js 상단 주석의 근본
// 원인과 동일). 단일 파일 파싱이라 scanFiles()의 cross-include XML 메타는 필요 없고, 가상 abapGit
// 이름으로만 바꿔주면 된다. 반환하는 `file`은 항상 원본 fileName 그대로다.
function virtualPreviewName(fileName, source) {
  if (isAbapGitNamed(fileName)) return fileName;
  const lastSlash = fileName.lastIndexOf("/");
  const dir = lastSlash >= 0 ? fileName.slice(0, lastSlash + 1) : "";
  const base = fileName.slice(lastSlash + 1).replace(/\.abap$/i, "");
  return `${dir}${base}.${detectAbapGitType(source)}.abap`;
}

export function parsePreview(source, fileName = "source.prog.abap") {
  const virtualName = virtualPreviewName(fileName, source);
