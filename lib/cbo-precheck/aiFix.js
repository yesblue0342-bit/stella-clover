...
import { providerStatus, callModelWithFallback } from "../ai-connection/providers.js";
...
//
// keyUsable()이 mode==="cli"인 provider의 API 키를 일부러 건너뛰는 이유: 공용 모듈 callModel()은
// mode==="cli"면 미인증이어도 절대 API 키 경로로 새지 않는다(providers.js의 의도된 보안 규칙 — "cli 모드는
// API 키 경로로 절대 새지 않는다"). 이 함수가 그 provider의 저장된 키를 보고 "api-key"로 골라버리면,
// 실제 callModel() 호출은 여전히 cli 분기를 타 "로그인 만료" 오류로 실패한다(버튼은 활성인데 클릭하면
// 실패하는 불일치) — 그래서 이 함수도 callModel()과 동일하게 "cli 모드면 키를 보지 않는다"를 따른다.
export async function pickAiConnection() {
  ...
  const keyUsable = (provider) => byProvider[provider]?.mode !== "cli" && byProvider[provider]?.hasKey;
  ...
  if (keyUsable("anthropic")) return { provider: "anthropic", model: DEFAULT_MODEL.anthropic, via: "api-key" };
  if (keyUsable("openai")) return { provider: "openai", model: DEFAULT_MODEL.openai, via: "api-key" };
  return null;
}
...
export async function suggestFix({ fileName, source, issues }) {
  ...
  const raw = await callModelWithFallback({ provider: picked.provider, model: picked.model, system: SYSTEM, user: buildPrompt(fileName, source, issues), json: false });
