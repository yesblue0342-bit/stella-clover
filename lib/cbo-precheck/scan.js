...
//
// 네이밍 어댑터(WORK_REPORT.md 2026-07-14 Phase 0/1 세션 참고):
//   abaplint(@abaplint/core `AbstractFile.getObjectType()`)는 파일명을 "."으로 split해 두 번째 조각을
//   대문자로 오브젝트 타입으로 쓴다(`x.prog.abap`→"PROG"). `ZAQMR0130.abap`처럼 단순 확장자만 쓰면
//   split[1]="ABAP"이 되어 등록된 타입(PROG/CLAS/INTF/FUGR/...) 어디에도 안 걸리고 `UnknownObject`로
//   떨어진다 — `UnknownObject`는 RulesRunner가 사실상 검사하지 않아 **항상 이슈 0건**이 나온다(실측:
//   440건 vs 0건, 코드가 깨끗해서가 아니라 타입 인식 실패로 조용히 스킵된 것). `virtualizeFiles()`가
//   이런 파일을 스캔 직전 가상 abapGit 파일명(`<원본>.prog.abap`/`.clas.abap`)으로만 매핑해 Registry에
//   넘기고, 결과의 `issue.file`은 다시 원본 파일명으로 역치환한다 — 실제 디스크/메모리 사본은 만들지
//   않고 Registry 안에서만 이름을 바꾸므로(기존 MemoryFile 기반 인메모리 구조 재사용) 정리할 임시
//   디렉토리 자체가 없다.
//
...
export function detectAbapGitType(content) {
  ...
  if (/(^|\n)\s*(REPORT|PROGRAM)\s+\S/i.test(code)) return "prog";
  if (/\bCLASS\s+\S+\s+DEFINITION\s+PUBLIC\b/i.test(code)) return "clas";
  return "prog";
}
...
// abapGit 네이밍이 아닌 .abap 파일을 스캔 직전 가상 파일명(원본 디렉토리 유지, 베이스네임만
// `<원본>.<타입>.abap`)으로 매핑한다. 실제 사본은 만들지 않음 — Registry에 넘길 이름만 바꾼다.
// prog 타입인데 REPORT/PROGRAM 문이 없는 파일(=include)에는 가상 `.prog.xml`을 함께 등록한다.
function virtualizeFiles(list) {
  const virtualByOriginal = new Map();
  const originalByVirtual = new Map();
  const virtualXml = [];
  for (const f of list) {
    if (!isAbapSource(f.name) || isAbapGitNamed(f.name)) {
      virtualByOriginal.set(f.name, f.name);
      originalByVirtual.set(f.name, f.name);
      continue;
    }
    const lastSlash = f.name.lastIndexOf("/");
    const dir = lastSlash >= 0 ? f.name.slice(0, lastSlash + 1) : "";
    const base = f.name.slice(lastSlash + 1).replace(/\.abap$/i, "");
    const type = detectAbapGitType(f.content);
    const virtualName = `${dir}${base}.${type}.abap`;
    virtualByOriginal.set(f.name, virtualName);
    originalByVirtual.set(virtualName, f.name);
    if (type === "prog" && !/(^|\n)\s*(REPORT|PROGRAM)\s+\S/i.test(stripComments(f.content))) {
      virtualXml.push(`${dir}${base}.prog.xml`);
    }
  }
