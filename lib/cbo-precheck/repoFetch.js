...
function isTargetFile(name) {
  ...
  return { isAbap, isDdic, isDict, isTarget: isAbap || isDdic || isDict };
}
...
export async function collectAbapFiles(root) {
  ...
  if (rootStat && rootStat.isFile()) {
    const { isTarget, isDdic, isDict } = isTargetFile(path.basename(root));
    if (isTarget && rootStat.size <= MAX_FILE_BYTES) {
      const content = await fs.readFile(root, "utf8");
      files.push({ name: path.basename(root), content, isDdic, isDict });
    }
    return files;
  }
  ...
  async function walk(dir) {
    ...
    for (const entry of entries) {
      ...
      const { isTarget, isDdic, isDict } = isTargetFile(relName);
      if (!isTarget) continue;
      const stat = await fs.stat(full);
      if (stat.size > MAX_FILE_BYTES) continue;
      const content = await fs.readFile(full, "utf8");
      files.push({ name: relName, content, isDdic, isDict });
    }
  }
  await walk(root);
  return files;
