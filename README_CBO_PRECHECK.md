...
URL / 브랜치 / **파일 경로**(예: `260707_QM023_ZAQMR0130/_abap/ZAQMR0130_S01.abap`)를 입력하고
[미리보기 생성]을 누르면 렌더링합니다(`action=preview-direct`). `GITHUB_TOKEN` 없이도 동작합니다(SSH
배포키 clone만 사용 — 스캔과 동일한 전제). 비abapGit 네이밍 파일도 위와 동일한 어댑터가 적용되어 정상
파싱됩니다. **지정한 파일이 속한 폴더 전체를 clone**합니다(파일 하나만이 아님) — 아래 "TEXT 라벨 자동
치환"/"INCLUDE 자동 병합" 절에서 설명하는 같은 폴더의 형제 파일(TEXTS 문서·INCLUDE 대상)을 찾기 위함입니다.
...
- **저장소에 DDIC XML도 `dictionary/` 문서도 없는 커스텀 테이블은 "찾을 수 없음"으로 계속 보고됩니다.**
  `ICON` 타입풀(`icon_create` 등)이나 방법 1/2 어느 쪽으로도 정의되지 않은 Z테이블을 참조하면 abaplint가
  진짜 결함처럼 보고합니다 — 이 도구가 SAP 시스템 전체 DDIC/타입풀에 접근할 수 없어서 생기는 기대된
  한계입니다("DDIC 정의 넣는 법" 절 참고, 방법 1(export) 또는 방법 2(`dictionary/` 문서 추가) 중 하나로
  넣으면 해소됩니다).
