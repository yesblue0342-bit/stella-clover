...
function gitEnv() {
  ...
  if (!token) return env;
  // ★ git-over-HTTPS 인증은 Basic 이어야 한다(회귀 방지). 과거 `Authorization: Bearer <token>` 는
  //   GitHub git smart-HTTP 에서 거부되어 clone 이 사용자명 프롬프트로 빠지고
  //   "could not read Username for 'https://github.com': terminal prompts disabled" 로 실패했다.
  //   PAT 는 Basic 의 비밀번호로 동작하므로 x-access-token:<token> 을 base64 로 싣는다(토큰은 URL/로그 미노출).
...
function remoteUrl() {
  return process.env.GITHUB_TOKEN ? HTTPS_REMOTE : SSH_REMOTE;
}
...
export async function ensureRepo() {
  try { await fs.access(path.join(REPO_PATH, ".git")); }
    ...
    await exec("git", ["clone", "--branch", BRANCH, remoteUrl(), REPO_PATH], { env: gitEnv(), timeout: 120000, windowsHide: true });
  }
