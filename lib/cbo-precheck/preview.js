...
import { extractScreenNumbers, findAllModules, buildScreenInfo, buildSalvScreenInfo } from "./dynproPreview.js";
import { detectAbapObject } from "./abapObject.js";
import { buildRfcPreview } from "./rfcPreview.js";
...
export function buildPreview(fileName, source, siblingFiles = []) {
  ...
  if (object.type === "function-module") {
    return buildRfcPreview(fileName, source, siblingFiles);
  }
  ...
  let screens = screenNumbers.map((screenNo) => buildScreenInfo({ screenNo, source: mergedSource, textsMap, modules, alvColumnsByTable }));
  let flow = screens.length ? ["Selection Screen(1000)", ...screenNumbers.map((n) => `Screen ${n}`)] : [];

  // CALL SCREEN이 전혀 없는 SALV 전체화면 ALV(예: ZAQMR0131) — 커스텀 Dynpro가 없어 위 screenNumbers가
  // 비어 있을 때만 시도한다(Dynpro 화면이 하나라도 있으면 그게 우선이고, 이 분기는 건드리지 않음 — 회귀 없음).
  if (!screens.length) {
    const salvInfo = buildSalvScreenInfo(mergedSource);
    if (salvInfo) {
      screens = [{
        screenNo: null,
        salv: true,
        title: salvInfo.title,
        pfStatus: null,
        functionCodes: [],
        modules: { output: [], input: [], fallback: false },
        hasAlvGrid: true,
        alvColumns: salvInfo.columns,
        toolbarButtons: [],
      }];
  ...
  return { ...result, objectType: object.type === "report" ? "report" : object.type, objectName: object.name, mergedFiles, warnings, screens, flow };
}
