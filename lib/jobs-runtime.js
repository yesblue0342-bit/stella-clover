...
import { getPool, sql, CREATE_JOBS, parseJson } from "../api/_db.js";
  ...
  await pool.request()
    ...
    try {
      await pool.request().input("id", sql.BigInt, id).input("e", sql.NVarChar(sql.MAX), String((e && e.message) || e).slice(0, 1000))
        .query("UPDATE transcribe_jobs SET status='error', error_msg=@e, updated_at=SYSUTCDATETIME() WHERE job_id=@id AND status<>'done'");
...
export async function recover() {
  try {
    ...
    const r = await pool.request()
      .query(`${CREATE_JOBS} SELECT job_id FROM transcribe_jobs WHERE status IN ('processing','summarizing') ORDER BY job_id ASC`);
