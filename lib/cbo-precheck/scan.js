...
//
// dictionary 문서 기반 합성 DDIC(WORK_REPORT.md 2026-07-14 "dictionary→DDIC" 세션 참고):
//   0Program 일부 프로그램 폴더는 커스텀 DDIC 테이블 정의를 abapGit TABL XML이 아니라 `dictionary/*.md`
//   또는 `dictionary/*.html` 사람이 읽는 문서로만 제공한다 — 이 경우 abaplint가 해당 테이블 필드 참조를
//   `unknown_types`로 오탐한다(실측: `TYPE zaqmt0132-werks` 등). `dictParser.js`가 두 문서 포맷을 파싱하고
//   `dictToTabl.js`가 합성 TABL XML을 만들어, 스캔 직전 이 Registry에만 추가로 등록한다(실제 `ddic/` 폴더에
//   파일을 쓰지 않음 — 기존 include 가상 XML과 동일한 인메모리 패턴). 이미 실제 `ddic/*.tabl.xml`이 있는
//   테이블은 그 파일이 우선하므로 합성하지 않는다(중복/충돌 방지).
import abaplint from "@abaplint/core";
import { isDictionaryDoc, parseDictDoc } from "./dictParser.js";
import { dictToTablXml } from "./dictToTabl.js";
import { isTextsDoc } from "./textSymbols.js";
import { isKnownSapStandardSymbol, extractNotFoundSymbol } from "./sapStandardSymbols.js";
...
export function hasReportStatement(content) {
  const obj = detectAbapObject(content);
  return obj.type === "report";
}
...
export function detectAbapGitType(content) {
  ...
  if (/\bCLASS\s+\S+\s+DEFINITION\s+PUBLIC\b/i.test(stripAbapComments(content))) return "clas";
  return "prog";
}
...
function existingDdicTableNames(list) {
  const names = new Set();
  for (const f of list) {
    if (!isDdicXml(f.name)) continue;
    const m = String(f.content || "").match(/<TABNAME>\s*([A-Za-z0-9_]+)\s*<\/TABNAME>/i);
    if (m) names.add(m[1].toUpperCase());
  }
  return names;
}

// dictionary/*.md|*.html 문서를 파싱해 합성 TABL XML 목록을 만든다. 이미 실제 ddic/*.tabl.xml로
// 정의된 테이블(같은 TABNAME)은 건너뛴다 — 실제 DDIC export가 항상 우선한다. 파싱 실패(문서 구조가
// "DDIC Table"이 아니거나 필드 표를 못 찾음 — Lock Object/Message Class 문서 등)는 조용히 건너뛴다.
function synthesizeDictXml(list) {
  const existing = existingDdicTableNames(list);
  const seen = new Set();
  const synthesized = [];
  for (const f of list) {
    if (!isDictionaryDoc(f.name)) continue;
    let dict;
    try { dict = parseDictDoc(f.name, f.content); } catch { dict = null; }
    if (!dict || existing.has(dict.tableName) || seen.has(dict.tableName)) continue;
    seen.add(dict.tableName);
    try {
      synthesized.push({ name: `${dict.tableName.toLowerCase()}.tabl.xml`, content: dictToTablXml(dict) });
    } catch { /* 합성 실패한 문서는 건너뛴다(단일 파일 실패가 전체 스캔을 막지 않음) */ }
  }
  return synthesized;
}

// files: [{ name, content }] — ABAP 소스(.abap) + DDIC XML(.xml) + dictionary 문서(.md/.html) 전부
// 여기로 전달. scan 대상이 아닌 DDIC/의존성/dictionary 문서도 같은 Registry 에 추가해야 타입 해석이 된다.
export function scanFiles({ files, config } = {}) {
  const rawList = Array.isArray(files) ? files : [];
  if (!rawList.length) return { issues: [], fileCount: 0 };

  const synthesizedDdic = synthesizeDictXml(rawList);
  // TEXTS.txt(텍스트 심볼/선택화면 텍스트 문서, lib/cbo-precheck/textSymbols.js)는 ABAP 소스가 아니라
  // 미리보기 라벨 치환용 참고 문서일 뿐이다 — abaplint Registry에 넣으면 의미 없는 오브젝트로 등록될 수
  // 있으므로 dictionary 문서와 동일하게 스캔 대상에서 제외한다.
  const list = rawList.filter((f) => !isDictionaryDoc(f.name) && !isTextsDoc(f.name) && !isRfcDdicDoc(f.name));

  const { virtualByOriginal, originalByVirtual, virtualXml } = virtualizeFiles(list);
  const baseConfig = config || buildConfig();
  ...
  function runOnce(cfg) {
    ...
    for (const d of synthesizedDdic) {
      reg.addFile(new MemoryFile(d.name, d.content));
    }
    return reg.findIssues().map(normalizeIssue);
  }

  const scannedNames = new Set(list.filter((f) => isScannable(f.name)).map((f) => f.name));

  // 1차 스캔에서 SAP 표준 심볼(TYPE-POOLS icon 등, WORK_REPORT.md Phase 1 참고)로 보이는 미해석 식별자를
  // 뽑아 `syntax.globalConstants`(abaplint 공식 확장점)에 추가한 뒤 2차로 다시 스캔한다 — 실제 미선언
  // 변수(check_syntax)는 이 패턴에 매치되지 않으므로 그대로 검출된다(회귀 없음, sapStandardSymbols.js 참고).
  let rawIssues = runOnce(baseConfig);
  const existingConstants = new Set((baseConfig.syntax?.globalConstants || []).map((s) => s.toLowerCase()));
  const discovered = new Set();
  for (const issue of rawIssues) {
    if (issue.rule !== "check_syntax") continue;
    const symbol = extractNotFoundSymbol(issue.message);
    if (!symbol || existingConstants.has(symbol.toLowerCase())) continue;
    if (isKnownSapStandardSymbol(symbol)) discovered.add(symbol);
  }
  if (discovered.size) {
    const augmented = {
      ...baseConfig,
      syntax: { ...baseConfig.syntax, globalConstants: [...(baseConfig.syntax?.globalConstants || []), ...discovered] },
    };
    rawIssues = runOnce(augmented);
  }
