...
import { buildMinutesSystemPrompt, buildPartialSystemPrompt, needsMapReduce, splitTranscript, meetingDateFromName, resolveMeetingTitle, titleFromFileName } from "../api/_meeting.js";
...
export async function generateMinutes({ transcript, lang = "ko", audioFileName = "", fileDate = "", userInstruction = "" }) {
  ...
  // 제목 우선순위: 업로드 파일명(사용자가 직접 정한 제목) > AI 요약 추출 제목(파일명 없을 때 폴백).
  const tm = summary.match(/##\s*회의 제목\s*\n+\s*([^\n]+)/);
  const fileTitle = titleFromFileName(audioFileName);
  const title = resolveMeetingTitle(fileTitle || (tm ? tm[1] : ""), new Date());
